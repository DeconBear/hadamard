...
describe('PowerShell tool', () => {
  it('executes a PowerShell command on Windows', async () => {
    ...
    const cwd = await createTempDir('hadamard-powershell-tool-');
    const tool = createPowerShellTool();
    const result = await tool.execute(
      { command: "Write-Output 'hadamard-ok'" },
      createContext(cwd),
    ) as { stdout: string; stderr: string; exitCode: number };

    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('hadamard-ok');
...
describe('Bash tool', () => {
  it('executes commands in the tool context cwd', async () => {
    const cwd = await createTempDir('hadamard-bash-tool-');
  ...
  it('provides POSIX shell commands on Windows', async () => {
    const cwd = await createTempDir('hadamard-bash-posix-');
...
describe('NotebookEdit tool', () => {
  it('replaces and inserts notebook cells', async () => {
    const cwd = await createTempDir('hadamard-notebook-tool-');
