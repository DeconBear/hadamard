...
import { createHadamardFileTools } from '../src/index.js';
...
describe('Hadamard Runtime parity file tools', () => {
  it('reads, edits, and writes files with upstream-style read-before-write safeguards', async () => {
    ...
    const tools = createHadamardFileTools({ cwd });
  ...
  it('applies sandbox read roots to Glob and Grep search paths', async () => {
    const root = await createTempDir('hadamard-parity-sandbox-search-');
    const workspace = path.join(root, 'workspace');
    const outside = path.join(root, 'outside');
    await Promise.all([mkdir(workspace), mkdir(outside)]);
    await writeFile(path.join(outside, 'secret.txt'), 'secret\n', 'utf8');
    const tools = createHadamardFileTools({ cwd: workspace });
    const context = {
