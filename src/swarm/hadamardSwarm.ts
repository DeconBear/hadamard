import type {
  ...
  HadamardMailboxMessage,
  HadamardSwarmRunResult,
  HadamardSwarmRuntimeContext,
  HadamardTeammateRecord,
  HadamardTeammateTranscript,
  CreateHadamardSwarmOptions,
  CreateHadamardTeammateOptions,
...
interface HadamardSwarmBindings {
  ...
  getBackgroundTask(taskId: string): Promise<HadamardBackgroundTaskRecord | undefined>;
}

interface ContinueHadamardTeammateOptions {
  prompt?: string;
  maxPasses?: number;
}
...
export class HadamardSwarmTeammateHandle {
  constructor(
    private readonly team: HadamardSwarmTeam,
    readonly name: string,
  ) {}

  state(): Promise<HadamardTeammateRecord | undefined> {
    return this.team.getTeammate(this.name);
  }

  run(prompt: string): Promise<HadamardSwarmRunResult> {
  ...
  inbox(): Promise<HadamardMailboxMessage[]> {
    return this.team.inbox(this.name);
  }

  message(text: string, from?: string): Promise<HadamardMailboxMessage> {
  ...
  continueFromMailbox(
    options: ContinueHadamardTeammateOptions = {},
  ): Promise<HadamardSwarmRunResult | undefined> {
    return this.team.continueFromMailbox(this.name, options);
  }

  recover(): Promise<HadamardTeammateRecord> {
    return this.team.recover(this.name);
  }

  transcript(): Promise<HadamardTeammateTranscript> {
    return this.team.transcript(this.name);
  }

  reenter(options: ContinueHadamardTeammateOptions = {}): Promise<HadamardSwarmRunResult | undefined> {
    return this.team.reenter(this.name, options);
  }
}

export class HadamardSwarmTeam {
  private readonly mailboxContinuations = new Map<
    string,
    Promise<HadamardSwarmRunResult | undefined>
  >();
  private runtimeContext: HadamardSwarmRuntimeContext = {};

  constructor(
    private readonly bindings: HadamardSwarmBindings,
  ...
  async spawn(options: CreateHadamardTeammateOptions): Promise<HadamardSwarmRunResult> {
    const createdAt = nowIso();
    const session = await this.bindings.createAgentSession(options.agent, {
      title: `${options.name}: ${options.prompt.slice(0, 80)}`,
      metadata: {
        __hadamardSwarmTeam: this.name,
        __hadamardTeammateName: options.name,
      },
    });
  ...
  setRuntimeContext(context: HadamardSwarmRuntimeContext = {}): void {
    this.runtimeContext = {
      hooks: context.hooks,
  ...
  async listTeammates(): Promise<HadamardTeammateRecord[]> {
    return this.syncTeammateStatuses(await this.teammateStore.list(this.name));
  }

  teammate(name: string): HadamardSwarmTeammateHandle {
    return new HadamardSwarmTeammateHandle(this, name);
  }

  async getTeammate(name: string): Promise<HadamardTeammateRecord | undefined> {
  ...
  async run(name: string, prompt: string): Promise<HadamardSwarmRunResult> {
  ...
  async continueFromMailbox(
    ...
    const continuation = (async () => {
      ...
      let lastResult: HadamardSwarmRunResult | undefined;
  ...
  async continueAllFromMailbox(
    options: ContinueHadamardTeammateOptions = {},
  ): Promise<HadamardSwarmRunResult[]> {
    const results: HadamardSwarmRunResult[] = [];
    const maxPasses = Math.max(1, options.maxPasses ?? 8);
  ...
  async recover(name: string): Promise<HadamardTeammateRecord> {
    const teammate = await this.requireTeammate(name);
    const recovered: HadamardTeammateRecord = {
      ...teammate,
      status: 'idle',
      taskId: undefined,
  ...
  async transcript(name: string): Promise<HadamardTeammateTranscript> {
    const teammate = await this.requireTeammate(name);
    const session = await this.bindings.resumeSession(teammate.sessionId);
  ...
  async runBackground(
    name: string,
    prompt: string,
    _options: WaitForHadamardBackgroundTaskOptions = {},
  ): Promise<HadamardBackgroundTaskRecord> {
    const teammate = await this.requireTeammate(name);
    const session = await this.bindings.resumeSession(teammate.sessionId);
    await this.applyRuntimeContext(session);
  ...
  async broadcast(text: string): Promise<HadamardMailboxMessage[]> {
    const teammates = await this.listTeammates();
    return Promise.all(
  ...
  inbox(recipient = this.leader): Promise<HadamardMailboxMessage[]> {
    return this.mailboxStore.list(this.name, recipient);
  }

  drainInbox(recipient = this.leader): Promise<HadamardMailboxMessage[]> {
    return this.mailboxStore.drain(this.name, recipient);
  }

  async waitForIdle(): Promise<HadamardTeammateRecord[]> {
    while (true) {
      if (this.continuous) {
  ...
  private async requireTeammate(name: string): Promise<HadamardTeammateRecord> {
  ...
  private async syncTeammateStatuses(
    teammates: HadamardTeammateRecord[],
  ): Promise<HadamardTeammateRecord[]> {
    const synced: HadamardTeammateRecord[] = [];
    ...
    for (const teammate of teammates) {
      ...
      const updated: HadamardTeammateRecord = {
        ...teammate,
        status:
...
export class HadamardSwarmApi {
  ...
  createTeam(options: CreateHadamardSwarmOptions): HadamardSwarmTeam {
    return new HadamardSwarmTeam(
      this.bindings,
      this.teammateStore,
      this.mailboxStore,
      options.name,
...
function composeTeammateInput(
  mailboxMessages: HadamardMailboxMessage[],
  prompt: string,
...
function formatTeammateMailboxMessages(messages: HadamardMailboxMessage[]): string | undefined {
