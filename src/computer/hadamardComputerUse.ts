...
export interface HadamardComputerUseToolkit {
  tools: AgentToolDefinition[];
  mcpServer: LocalMcpServerDefinition;
}

export const HADAMARD_COMPUTER_USE_WORKFLOW_ACTIONS = [
  'open_url',
  'focus_window',
  'type_text',
  'keypress',
...
type HadamardComputerWorkflowStep = z.infer<typeof workflowStepSchema>;
...
async function executeWorkflowStep(
  ...
  step: HadamardComputerWorkflowStep,
  workspaceDir: string,
  signal?: AbortSignal,
...
export function createHadamardComputerUseTools(
  options: CreateHadamardComputerUseOptions = {},
): AgentToolDefinition[] {
  const executor = options.executor ?? createDefaultHadamardComputerUseExecutor();
...
export function createHadamardComputerUseMcpServer(
  options: CreateHadamardComputerUseOptions = {},
): LocalMcpServerDefinition {
  return {
    kind: 'local',
    name: options.serverName ?? 'hadamard-computer-use',
    prefix: options.prefix ?? 'computer',
    tools: createHadamardComputerUseTools(options),
  };
}

export function createHadamardComputerUseToolkit(
  options: CreateHadamardComputerUseOptions = {},
): HadamardComputerUseToolkit {
  return {
    tools: createHadamardComputerUseTools(options),
    mcpServer: createHadamardComputerUseMcpServer(options),
  };
}
