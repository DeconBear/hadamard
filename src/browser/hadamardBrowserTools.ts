...
import {
  ...
  type HadamardBrowserSessionOptions,
} from './hadamardBrowserSession.js';

export interface HadamardBrowserUseToolkit {
  tools: AgentToolDefinition[];
  mcpServer: LocalMcpServerDefinition;
  session: HadamardBrowserSession;
}

export interface HadamardBrowserSessionLike {
...
export function sessionOptionsFromBrowserUse(
  options: CreateHadamardBrowserUseOptions = {},
...
export function createHadamardBrowserTools(
  options: CreateHadamardBrowserUseOptions = {},
): AgentToolDefinition[] {
  const session: HadamardBrowserSessionLike =
    options.session ?? new HadamardBrowserSession(sessionOptionsFromBrowserUse(options));
...
export function createHadamardBrowserUseMcpServer(
  options: CreateHadamardBrowserUseOptions = {},
): LocalMcpServerDefinition {
  return {
    kind: 'local',
    name: options.serverName ?? 'hadamard-browser-use',
    prefix: options.prefix ?? 'browser',
    tools: createHadamardBrowserTools(options),
  };
}

export function createHadamardBrowserUseToolkit(
  options: CreateHadamardBrowserUseOptions = {},
): HadamardBrowserUseToolkit {
  const session =
    (options.session as HadamardBrowserSession | undefined) ??
    new HadamardBrowserSession(sessionOptionsFromBrowserUse(options));
  const next: CreateHadamardBrowserUseOptions = { ...options, session };
  return {
    tools: createHadamardBrowserTools(next),
    mcpServer: createHadamardBrowserUseMcpServer(next),
    session,
  };
}
