...
export async function probeExternalCliAuth(
  ...
  try {
    executable = options.executable
      ?? await (options.resolveExecutable ?? resolveNativeCliExecutable)(runtime);
  } catch {
    return {
      runtime,
      state: 'missing',
...
export async function resolveNativeCliExecutable(runtime: ExternalCliAuthRuntime): Promise<string> {
  const envVar = `HADAMARD_${runtime.toUpperCase()}_PATH`;
  const loaded = getLoadedJsonConfig();
  const configured = loaded?.env?.[envVar]
    ?? process.env[envVar]
    ?? configuredRuntimePath(loaded?.raw, runtime);
  if (configured) {
    if (await pathExists(configured)) return configured;
    throw new Error(`${envVar} (${configured}) was not found.`);
  }

  const executable = await findExecutableOnPath(nativeCliBinary(runtime));
  if (executable) return executable;
  throw new Error(`No ${runtime} CLI executable was found on PATH.`);
}

function nativeCliBinary(runtime: ExternalCliAuthRuntime): string {
  return runtime === 'cursor' ? 'cursor-agent' : runtime;
}
...
function configuredRuntimePath(
  ...
  const entry = (providers as Record<string, unknown>)[runtime];
  if (!entry || typeof entry !== 'object') return undefined;
  const configuredPath = (entry as { path?: unknown }).path;
  return typeof configuredPath === 'string' && configuredPath ? configuredPath : undefined;
}
