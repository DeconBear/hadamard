#!/usr/bin/env node
/**
 * Hadamard — Interactive terminal agent.
 *
...
import {
  ...
  type HadamardPermissionMode,
  type HadamardToolApprover,
...
const CONFIG_PATH = process.argv[3] ?? path.join(resolveHadamardHome(), 'settings.json');
...
const SYSTEM_PROMPT =
  `You are Hadamard, an interactive CLI agent. Working directory: ${WORK_DIR}\n\n` +
  ...
  for (let attempt = 1; attempt <= MANAGED_PLUGIN_FINAL_CLOSE_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      process.stderr.write(
        `[hadamard-react] warning: managed plugin cleanup attempt ${attempt}/` +
...
async function main() {
  ...
  const coreTools = createHadamardCoreTools({
    cwd: WORK_DIR,
  ...
  try {
    ...
    else await loadDefaultHadamardSettings();
  } catch (e) {
    if (userSuppliedConfig) {
      ...
      process.stderr.write(
        ...
        `  ${(e as Error).message}\n\n` +
        `${C.d}Hadamard refused to start with a bad explicit config. Fix the file or omit the path to use defaults.${C.r}\n`,
      );
      process.exit(2);
    }
    // Default-settings path: tolerate and warn. A missing ~/.hadamard/settings.json
  ...
  const managedPluginRuntime = await resolveHadamardSettingsStore({
    ...
    if (t.startsWith('/')) {
      ...
      switch (cmd) {
        ...
        case 'permissions': {
          ...
          if (!PERMISSION_MODES.has(requested as HadamardPermissionMode)) {
            process.stdout.write(
          ...
          await session.setPermissionContext({
            mode: requested as HadamardPermissionMode,
        ...
        case 'workflows': {
          ...
          if (!sub || sub === 'list') {
            ...
            if (workflows.length === 0) {
              process.stdout.write(`${C.d}No saved workflows. Save scripts to .hadamard/workflows/${C.r}\n\n`);
            } else {
        ...
        case 'manager': {
          ...
          if (sub === 'schedule') {
            ...
            if (tasks.length === 0) {
              process.stdout.write(`${C.d}No manager schedules. Add kind:"manager" tasks to .hadamard/scheduled-tasks.json${C.r}\n\n`);
              return;
            }
  ...
  async function shutdown(code: number, closeReadline = true): Promise<void> {
    ...
    try {
      ...
      exitCode = 1;
      process.stderr.write(
        `[hadamard-react] ERROR: ${errorMessage(error)} ` +
    ...
    try {
      ...
      exitCode = 1;
      process.stderr.write(`[hadamard-react] ERROR: SDK cleanup failed: ${errorMessage(error)}\n`);
    }
