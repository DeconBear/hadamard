...
/**
 * Run an operation with an optional wall-clock deadline and a derived signal.
 * A finite race rejects even when a third-party callback ignores AbortSignal;
 * parent cancellation remains active when no deadline is configured.
 */
export async function withDeadline<T>(
  ...
  if (timeoutMs === undefined) {
    ...
    if (!parentSignal) {
      return operation({ signal: new AbortController().signal, timeoutMs, scope });
    }
    return new Promise<T>((resolve, reject) => {
      ...
      const onAbort = () => finish(() => reject(toAbortError(parentSignal.reason)));
      parentSignal.addEventListener('abort', onAbort, { once: true });
      let pending: Promise<T>;
      try {
        pending = Promise.resolve(operation({ signal: parentSignal, timeoutMs, scope }));
      } catch (error) {
        finish(() => reject(error));
        return;
      }
