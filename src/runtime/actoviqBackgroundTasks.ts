...
export interface ReserveActoviqBackgroundTaskInputResult {
  task: ActoviqBackgroundTaskRecord;
  input: ActoviqBackgroundTaskQueuedInput;
  accepted: boolean;
  queued: boolean;
  rejected?: string;
}
...
function isTerminalTask(task: ActoviqBackgroundTaskRecord): boolean {
  return (
...
function isOwnerLeaseFresh(task: ActoviqBackgroundTaskRecord): boolean {
...
function isTaskOwnerAlive(task: ActoviqBackgroundTaskRecord): boolean {
...
export class ActoviqBackgroundTaskManager {
  ...
  async launch(
    ...
    let configuredTask: ActoviqBackgroundTaskRecord | undefined;
    try {
  ...
  async reserveInput(
    taskId: string,
    input: ActoviqBackgroundTaskQueuedInput,
  ): Promise<ReserveActoviqBackgroundTaskInputResult> {
    let accepted = false;
    let queued = false;
  ...
  async drainInputs(taskId: string): Promise<ActoviqBackgroundTaskQueuedInput[]> {
    let drained: ActoviqBackgroundTaskQueuedInput[] = [];
  ...
  private async mutateTask(
    taskId: string,
    updater: (task: ActoviqBackgroundTaskRecord) => ActoviqBackgroundTaskRecord,
  ): Promise<ActoviqBackgroundTaskRecord> {
