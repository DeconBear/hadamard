...
export const HADAMARD_EXECUTION_ID_KEY = '__hadamardExecutionId';
export const HADAMARD_ROOT_EXECUTION_ID_KEY = '__hadamardRootExecutionId';
export const HADAMARD_PARENT_EXECUTION_ID_KEY = '__hadamardParentExecutionId';
export const HADAMARD_AGENT_PATH_KEY = '__hadamardAgentPath';
...
export function resolveAgentExecutionIdentity(
  ...
  const executionId =
    metadataString(sessionMetadata, runMetadata, HADAMARD_EXECUTION_ID_KEY) ?? sessionId;
  const parentExecutionId =
    metadataString(sessionMetadata, runMetadata, HADAMARD_PARENT_EXECUTION_ID_KEY) ?? null;
  const rootExecutionId =
    metadataString(sessionMetadata, runMetadata, HADAMARD_ROOT_EXECUTION_ID_KEY) ??
    (parentExecutionId ? parentExecutionId : executionId);
  const parentSessionId = options.session?.parentSessionId ??
    metadataString(sessionMetadata, runMetadata, '__hadamardParentSessionId') ?? null;
  const agentName =
    metadataString(sessionMetadata, runMetadata, '__hadamardAgentDefinition') ??
    metadataString(sessionMetadata, runMetadata, '__hadamardAgentName') ??
    'Hadamard';
  const nickname =
    metadataString(sessionMetadata, runMetadata, '__hadamardAgentName') ?? null;
  const parentPath = parentExecutionId ? '/root' : '';
  const canonicalPath =
    metadataString(sessionMetadata, runMetadata, HADAMARD_AGENT_PATH_KEY) ??
    (parentExecutionId
  ...
  return {
    ...
    agentName,
    nickname,
    role: metadataString(sessionMetadata, runMetadata, '__hadamardAgentRole') ?? null,
    kind: parentExecutionId ? 'subagent' : 'root',
    runtime: metadataString(sessionMetadata, runMetadata, '__hadamardRuntime')
      ?? options.runtime
      ?? 'hadamard',
...
export function serializeAgentExecutionIdentity(
  ...
  return {
    [HADAMARD_EXECUTION_ID_KEY]: identity.executionId,
    [HADAMARD_ROOT_EXECUTION_ID_KEY]: identity.rootExecutionId,
    ...(identity.parentExecutionId
      ? { [HADAMARD_PARENT_EXECUTION_ID_KEY]: identity.parentExecutionId }
      : {}),
    ...(identity.parentSessionId
      ? { __hadamardParentSessionId: identity.parentSessionId }
      : {}),
    [HADAMARD_AGENT_PATH_KEY]: identity.canonicalPath,
    __hadamardAgentName: identity.nickname ?? identity.agentName,
    ...(identity.role ? { __hadamardAgentRole: identity.role } : {}),
  };
}
 ...
 */
export class HadamardAgentExecutionsApi {
