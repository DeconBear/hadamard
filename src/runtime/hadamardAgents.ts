...
export function createHadamardRunToolState(): HadamardRunToolState {
  return { subagentFanout: 0 };
}

export interface HadamardAgentSessionLike {
  readonly id: string;
  send(input: string | MessageParam['content'], options?: AgentRunOptions): Promise<AgentRunResult>;
}

interface HadamardAgentBindings {
  listDefinitions: () => HadamardAgentDefinitionSummary[];
  getDefinition: (agent: string) => HadamardAgentDefinition | undefined;
  runDefinition: (
    agent: string,
  ...
  ) => Promise<HadamardAgentSessionLike>;
}

export function summarizeHadamardAgentDefinition(
...
export class HadamardAgentHandle {
  constructor(
    private readonly bindings: HadamardAgentBindings,
    readonly name: string,
    private readonly defaults: AgentRunOptions = {},
  ) {}

  definition(): HadamardAgentDefinition | undefined {
    return this.bindings.getDefinition(this.name);
  }

  summary(): HadamardAgentDefinitionSummary | undefined {
  ...
  createSession(options: SessionCreateOptions = {}): Promise<HadamardAgentSessionLike> {
    return this.bindings.createDefinitionSession(this.name, options);
  }
}

export class HadamardAgentsApi {
  constructor(private readonly bindings: HadamardAgentBindings) {}

  list(): HadamardAgentDefinitionSummary[] {
    return this.bindings.listDefinitions();
  }

  get(name: string): HadamardAgentDefinition | undefined {
    return this.bindings.getDefinition(name);
  }

  use(name: string, defaults: AgentRunOptions = {}): HadamardAgentHandle {
    return new HadamardAgentHandle(this.bindings, name, defaults);
  }
...
export function createHadamardTaskTool(options: {
  ...
  maxDepth?: number;
  maxFanout?: number;
}): AgentToolDefinition<HadamardTaskToolInput, HadamardTaskToolResult> {
  ...
  return tool(
    ...
    async (input, context) => {
      ...
      const parentDepth = readNonNegativeInteger(context.metadata.__hadamardAgentDepth) ?? 0;
      ...
      const allowedAgents = readStringArray(context.metadata.__hadamardAllowedAgents);
      ...
      const runState = getHadamardRunToolState(context.metadata);
      ...
      const delegation = {
        ...
        parentExecutionId:
          typeof context.metadata.__hadamardExecutionId === 'string'
            ? context.metadata.__hadamardExecutionId
            : context.sessionId ?? context.runId,
        rootExecutionId:
          typeof context.metadata.__hadamardRootExecutionId === 'string'
            ? context.metadata.__hadamardRootExecutionId
            : context.sessionId ?? context.runId,
        parentAgentPath:
          typeof context.metadata.__hadamardAgentPath === 'string'
            ? context.metadata.__hadamardAgentPath
            : '/root',
      };
...
function getHadamardRunToolState(metadata: Record<string, unknown>): HadamardRunToolState {
  const existing = metadata[HADAMARD_RUN_STATE_KEY];
  if (
    ...
    return existing as HadamardRunToolState;
  }
  const created = createHadamardRunToolState();
  metadata[HADAMARD_RUN_STATE_KEY] = created;
  return created;
}
...
function formatAvailableSubagents(agents: HadamardAgentDefinitionSummary[]): string {
...
function extractInheritedDelegationOptions(
  ...
  delete inheritedMetadata[HADAMARD_RUN_STATE_KEY];
  return {
    ...
    metadata: {
      ...
      ...(delegation.allowedAgents
        ? { __hadamardAllowedAgents: [...delegation.allowedAgents] }
        : {}),
    },
  };
}
