...
export async function loadHadamardAgentDefinitions(options: {
  homeDir: string;
  workDir: string;
  agentDirectories?: string[];
  loadDefaultAgentDirectories?: boolean;
}): Promise<HadamardAgentDefinition[]> {
  ...
  if (options.loadDefaultAgentDirectories !== false) {
    directories.push(
      ...
      { path: path.join(options.workDir, '.hadamard', 'agents'), source: 'project' },
    );
  }
...
async function loadAgentDirectory(
  ...
  const definitions: HadamardAgentDefinition[] = [];
      ...
      const fallbackReason =
        !parsed.frontmatter.name?.trim() && !path.basename(entry.name, path.extname(entry.name)).trim()
          ...
          : !parsed.body.trim()
            ...
            : 'missing description';
      console.warn(
        `[hadamard] Skipping agent definition "${path.basename(entry.name)}" in ${directory.path}: ${fallbackReason}. ` +
...
function createAgentDefinition(input: {
  ...
  return {
    ...
    memory: memory as HadamardAgentDefinition['memory'],
    background: parseBoolean(input.frontmatter.background),
    isolation: isolation as HadamardAgentDefinition['isolation'],
