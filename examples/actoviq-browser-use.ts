 ...
 * browser binary. Set ACTOVIQ_BROWSER_LIVE=1 to drive a real Chromium:
 *
 *   npx playwright install chromium
 *   ACTOVIQ_BROWSER_LIVE=1 npm run example:actoviq-browser-use
 */
import {
...
const live = process.env.ACTOVIQ_BROWSER_LIVE === '1';

const toolkit = live
  ? createActoviqBrowserUseToolkit({ headless: true })
  : createActoviqBrowserUseToolkit({
      session: {
        async navigate(url) {
