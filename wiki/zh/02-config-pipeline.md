...
2. process.env               (HADAMARD_* 变量)
3. ~/.hadamard/settings.json  (→ env 块)
4. 硬编码默认值               (最低优先级)
```
...
function resolveHadamardModelReference(model: string, tiers: HadamardModelTierConfig): string {
...
分级映射来自环境变量：`HADAMARD_DEFAULT_MIN_MODEL`、`HADAMARD_DEFAULT_MEDIUM_MODEL`、`HADAMARD_DEFAULT_MAX_MODEL`。
...
结果：`~/.hadamard/projects/<encoded-path>/sessions/`
...
1. **首次运行缺少 settings.json**：`loadDefaultHadamardSettings()` 返回空配置（不报错）
2. **显式配置路径但文件损坏**：`loadJsonConfigFile()` 抛出异常，CLI 以退出码 2 退出
3. **分级模型未配置环境变量**：`resolveHadamardModelReference()` 抛出 `ConfigurationError`
